...
@doc raw"""
    ...
    lobatto_legendre_points(T, s; IT=BigFloat)
...
roots by Rodrigues' formula. The roots are Newton-refined in the arithmetic `IT`
starting from the double precision approximations of
`FastGaussQuadrature.gausslobatto`, and the endpoints are set to exactly ``\pm 1``.
...
@doc raw"""
    ...
    lobatto_legendre_nodes(T, s; IT=BigFloat)
...
@doc raw"""
    ...
    lobatto_legendre_weights(T, s; IT=BigFloat)
...
```
"""
function lobatto_legendre_weights(::Type{T}, s::Integer; IT=BigFloat) where {T}
...
@doc raw"""
    ...
    LobattoLegendreQuadrature(T, s; IT=BigFloat, fast=false)
