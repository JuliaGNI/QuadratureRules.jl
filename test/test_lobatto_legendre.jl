...
@testset "$(rpad("Lobatto-Legendre",80))" begin
    ...
    @test_throws ErrorException lobatto_legendre_nodes(1; interval = SymmetricInterval())
    @test_throws ErrorException lobatto_legendre_weights(1)
    @test_throws ErrorException lobatto_legendre_weights(1; interval = SymmetricInterval())
    ...
    for s in 2:10
        ...
        @test lobatto_legendre_nodes(s; interval = SymmetricInterval()) == lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval())
        ...
        @test lobatto_legendre_weights(s; interval = SymmetricInterval()) == lobatto_legendre_weights(Float64, s; interval = SymmetricInterval())
        ...
        @test shift_nodes(lobatto_legendre_nodes(BigFloat, s; interval = SymmetricInterval())) == lobatto_legendre_nodes(BigFloat, s)
        @test shift_nodes(lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval()))  ≈  lobatto_legendre_nodes(Float64, s)
        @test unshift_nodes(lobatto_legendre_nodes(Float64, s)) ≈ lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval())
        @test lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval()) ≈ FastGaussQuadrature.gausslobatto(s)[1]

        # likewise the weights for [-1,+1] are primary and those for [0,1] are derived
        @test scale_weights(lobatto_legendre_weights(BigFloat, s; interval = SymmetricInterval())) == lobatto_legendre_weights(BigFloat, s)
        @test unscale_weights(lobatto_legendre_weights(Float64, s))      ≈  lobatto_legendre_weights(Float64, s; interval = SymmetricInterval())
        @test lobatto_legendre_weights(Float64, s; interval = SymmetricInterval()) ≈ FastGaussQuadrature.gausslobatto(s)[2]

        @test sum(lobatto_legendre_weights(Float64, s; interval = SymmetricInterval())) ≈ 2
        @test all(wᵢ > 0 for wᵢ in lobatto_legendre_weights(Float64, s; interval = SymmetricInterval()))

        # the outermost symmetric nodes are pinned exactly, and shift_nodes maps them
        # to exactly 0 and 1
        @test lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval())[begin] == -1
        @test lobatto_legendre_nodes(Float64, s; interval = SymmetricInterval())[end]   == +1
