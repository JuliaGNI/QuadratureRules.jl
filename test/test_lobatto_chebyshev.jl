...
@testset "$(rpad("Lobatto-Chebyshev",80))" begin
    ...
    for s in 2:10
        ...
        @test lobatto_chebyshev_nodes(Float64, s; interval = SymmetricInterval()) ==
              chebyshev_nodes(Float64, s, Val(2); interval = SymmetricInterval())
        @test lobatto_chebyshev_nodes(Float64, s) == chebyshev_nodes(Float64, s, Val(2))
        @test lobatto_chebyshev_nodes(s; interval = SymmetricInterval()) ==
              lobatto_chebyshev_nodes(Float64, s; interval = SymmetricInterval())
        @test lobatto_chebyshev_nodes(s) == lobatto_chebyshev_nodes(Float64, s)
        @test chebyshev_nodes(s, 2; interval = SymmetricInterval()) ==
              chebyshev_nodes(Float64, s, Val(2); interval = SymmetricInterval())
        @test chebyshev_nodes(s, 2) == chebyshev_nodes(Float64, s, Val(2))

        @test unshift_nodes(chebyshev_nodes(Float64, s, Val(2))) ≈
              chebyshev_nodes(Float64, s, Val(2); interval = SymmetricInterval())

        @test chebyshev_weights(Float64, s, Val(2)) ==
              weights(LobattoChebyshevQuadrature(s))
        @test lobatto_chebyshev_weights(Float64, s) == chebyshev_weights(Float64, s, Val(2))
        @test lobatto_chebyshev_weights(Float64, s; interval = SymmetricInterval()) ==
              chebyshev_weights(Float64, s, Val(2); interval = SymmetricInterval())
        @test lobatto_chebyshev_weights(s) == lobatto_chebyshev_weights(Float64, s)
        @test lobatto_chebyshev_weights(s; interval = SymmetricInterval()) ==
              lobatto_chebyshev_weights(Float64, s; interval = SymmetricInterval())
        @test chebyshev_weights(s, 2) == chebyshev_weights(Float64, s, Val(2))
        @test chebyshev_weights(s, 2; interval = SymmetricInterval()) ==
              chebyshev_weights(Float64, s, Val(2); interval = SymmetricInterval())

        # here the weights for [0,1] are primary and those for [-1,+1] are derived
        @test unscale_weights(chebyshev_weights(BigFloat, s, Val(2))) ==
              chebyshev_weights(BigFloat, s, Val(2); interval = SymmetricInterval())
        @test scale_weights(chebyshev_weights(Float64, s, Val(2); interval = SymmetricInterval())) ≈
              chebyshev_weights(Float64, s, Val(2))
        ...
        @test LobattoChebyshevQuadrature(BigFloat, s) ==
              ClenshawCurtisQuadrature(BigFloat, s)
