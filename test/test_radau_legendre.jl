...
@testset "$(rpad("Radau-Legendre",80))" begin
    ...
    for endpoint in (:left, :right)
        for s in 1:10
            ...
            @test radau_legendre_points(s, endpoint)        == radau_legendre_points(Float64, s, Val(endpoint))
            @test radau_legendre_nodes(s, endpoint)         == radau_legendre_nodes(Float64, s, Val(endpoint))
            @test radau_legendre_point_weights(s, endpoint) == radau_legendre_point_weights(Float64, s, Val(endpoint))
            ...
            @test shift_nodes(radau_legendre_points(BigFloat, s, Val(endpoint))) == radau_legendre_nodes(BigFloat, s, Val(endpoint))
            @test shift_nodes(radau_legendre_points(Float64, s, Val(endpoint)))  ≈  radau_legendre_nodes(Float64, s, Val(endpoint))
            @test unshift_nodes(radau_legendre_nodes(Float64, s, Val(endpoint))) ≈  radau_legendre_points(Float64, s, Val(endpoint))

            @test scale_weights(radau_legendre_point_weights(BigFloat, s, Val(endpoint))) == radau_legendre_weights(BigFloat, s, Val(endpoint))
            @test unscale_weights(radau_legendre_weights(Float64, s, Val(endpoint)))      ≈  radau_legendre_point_weights(Float64, s, Val(endpoint))

            @test sum(radau_legendre_point_weights(Float64, s, Val(endpoint))) ≈ 2
            @test all(wᵢ > 0 for wᵢ in radau_legendre_point_weights(Float64, s, Val(endpoint)))
    ...
    for s in 1:10
        ...
        @test radau_legendre_points(Float64, s, Val(:left))        ≈ FastGaussQuadrature.gaussradau(s)[1]
        @test radau_legendre_point_weights(Float64, s, Val(:left)) ≈ FastGaussQuadrature.gaussradau(s)[2]

        # the right variant is the reflection of the left one, exactly so
        @test radau_legendre_points(BigFloat, s, Val(:right))        == -reverse(radau_legendre_points(BigFloat, s, Val(:left)))
        @test radau_legendre_point_weights(BigFloat, s, Val(:right)) ==  reverse(radau_legendre_point_weights(BigFloat, s, Val(:left)))

        # the prescribed endpoint is pinned exactly, and shift_nodes maps it to exactly 0 or 1
        @test radau_legendre_points(Float64, s, Val(:left))[begin]  == -1
        @test radau_legendre_points(Float64, s, Val(:right))[end]   == +1
        ...
        @test radau_legendre_points(Float64, s, Val(:left))[end]   < +1
        @test radau_legendre_points(Float64, s, Val(:right))[begin] > -1
    end
