...
@testset "$(rpad("Radau-Legendre",80))" begin
    ...
    for endpoint in (:left, :right)
        for s in 1:10
            ...
            @test radau_legendre_nodes(s, endpoint; interval = SymmetricInterval())        == radau_legendre_nodes(Float64, s, Val(endpoint); interval = SymmetricInterval())
            @test radau_legendre_nodes(s, endpoint)         == radau_legendre_nodes(Float64, s, Val(endpoint))
            @test radau_legendre_weights(s, endpoint; interval = SymmetricInterval()) == radau_legendre_weights(Float64, s, Val(endpoint); interval = SymmetricInterval())
            ...
            @test shift_nodes(radau_legendre_nodes(BigFloat, s, Val(endpoint); interval = SymmetricInterval())) == radau_legendre_nodes(BigFloat, s, Val(endpoint))
            @test shift_nodes(radau_legendre_nodes(Float64, s, Val(endpoint); interval = SymmetricInterval()))  ≈  radau_legendre_nodes(Float64, s, Val(endpoint))
            @test unshift_nodes(radau_legendre_nodes(Float64, s, Val(endpoint))) ≈  radau_legendre_nodes(Float64, s, Val(endpoint); interval = SymmetricInterval())

            @test scale_weights(radau_legendre_weights(BigFloat, s, Val(endpoint); interval = SymmetricInterval())) == radau_legendre_weights(BigFloat, s, Val(endpoint))
            @test unscale_weights(radau_legendre_weights(Float64, s, Val(endpoint)))      ≈  radau_legendre_weights(Float64, s, Val(endpoint); interval = SymmetricInterval())

            @test sum(radau_legendre_weights(Float64, s, Val(endpoint); interval = SymmetricInterval())) ≈ 2
            @test all(wᵢ > 0 for wᵢ in radau_legendre_weights(Float64, s, Val(endpoint); interval = SymmetricInterval()))
    ...
    for s in 1:10
        ...
        @test radau_legendre_nodes(Float64, s, Val(:left); interval = SymmetricInterval())        ≈ FastGaussQuadrature.gaussradau(s)[1]
        @test radau_legendre_weights(Float64, s, Val(:left); interval = SymmetricInterval()) ≈ FastGaussQuadrature.gaussradau(s)[2]

        # the right variant is the reflection of the left one, exactly so
        @test radau_legendre_nodes(BigFloat, s, Val(:right); interval = SymmetricInterval())        == -reverse(radau_legendre_nodes(BigFloat, s, Val(:left); interval = SymmetricInterval()))
        @test radau_legendre_weights(BigFloat, s, Val(:right); interval = SymmetricInterval()) ==  reverse(radau_legendre_weights(BigFloat, s, Val(:left); interval = SymmetricInterval()))

        # the prescribed endpoint is pinned exactly, and shift_nodes maps it to exactly 0 or 1
        @test radau_legendre_nodes(Float64, s, Val(:left); interval = SymmetricInterval())[begin]  == -1
        @test radau_legendre_nodes(Float64, s, Val(:right); interval = SymmetricInterval())[end]   == +1
        ...
        @test radau_legendre_nodes(Float64, s, Val(:left); interval = SymmetricInterval())[end]   < +1
        @test radau_legendre_nodes(Float64, s, Val(:right); interval = SymmetricInterval())[begin] > -1
    end
