...
```@repl rules
symmetric_radau_legendre_nodes(3, :left)
symmetric_radau_legendre_nodes(3, :right)
```
...
finding is needed. [`symmetric_chebyshev_nodes`](@ref) provides the two kinds, and
...
```@repl rules
symmetric_chebyshev_nodes(5, 1)
symmetric_chebyshev_nodes(5, 2)
```
...
symmetric_lobatto_chebyshev_nodes(6) == symmetric_clenshaw_curtis_nodes(6)
```
...
underlying points, in the same way that [`symmetric_chebyshev_nodes`](@ref) does:
